...
var HabitParser = class {
  ...
  /**
   * Extrae las definiciones de hábitos del contenido
   */
  ...
  /**
   * Parsea un bloque individual de definición de hábito
   */
  ...
  /**
   * Parsea la sección TODO list - línea por línea para mayor robustez
   */
  ...
  /**
   * Detecta el año basándose en las entradas
   */
  detectYear(entries) {
...
var HabitGridView = class extends import_obsidian.ItemView {
   ...
   * @returns true si se renderizó un banner, false si no
   */
  ...
  /**
   * Renderiza la sección de estadísticas generales con gráfica
   */
  ...
  /**
   * Redibuja el gráfico cuando cambia el tamaño
   */
  redrawChart() {
    if (
      !this.chartCanvas ||
  ...
  /**
   * Maneja el hover sobre el gráfico
   */
  handleChartHover(e) {
    if (
      !this.chartCanvas ||
    ...
    if (dayIndex >= 0 && dayIndex < 30 && dayIndex < this.chartDays.length) {
      ...
      for (const habit of this.config.habits) {
        ...
        if (data && data[dayIndex] !== undefined) {
          tooltipText += `${habit.tema}: ${data[dayIndex]} días\n`;
          hasData = true;
        }
      }
  ...
  renderHabitGrid(container, habit) {
    ...
    streakBadge.setText(`Racha actual: ${stats.currentStreak} días`);
    ...
    // ===== FUNCIÓN DE ANIMACIÓN (DESPUÉS del renderizado) =====
  ...
  /**
   * Renderiza el grid del año completo
   */
  ...
  /**
   * Genera las semanas del año con información de cada día
   */
  ...
  /**
   * Calcula la intensidad (días completados) por semana para un hábito
   * Retorna un Map de weekIndex -> número de días completados esa semana
   */
  ...
  /**
   * Verifica si un día está completado para un hábito
   */
  ...
  calculateAdvancedStats(habit, basicStats) {
    ...
    // Encontrar el día con mejor porcentaje
    let topDay = 0;
    let topDayPercent = 0;
    const dayNames = [
      "Lunes",
    ...
    // Calcular tendencia (comparar últimas 2 semanas vs 2 anteriores)
  ...
  /**
   * Verifica qué badges están desbloqueados
   */
  checkBadges(habit, stats, monthSuccess) {
    const badges = [];

    // Badge: Racha de Fuego (7 días seguidos)
    ...
    // Badge: Diamante (30 días seguidos)
    ...
    // Badge: Relámpago (50 días en total)
    ...
    // Badge: Precisión (90% de éxito mensual)
    if (monthSuccess >= 90) {
      badges.push({
        id: "target",
    ...
    // Badge: Campeón (100 días en total)
    ...
    // Badge: Estrella (180 días en total)
    ...
    // Badge: Leyenda (365 días en total)
    ...
    // Badge: Consistencia (mejor racha > 28 días)
    ...
    // Badge: Artista (mes completo sin fallos - 30 días seguidos)
  ...
  renderCardBack(container, habit, basicStats, advancedStats) {
    ...
    title.innerHTML = `<span class="back-arrow">←</span> Análisis de Desempeño - ${habit.tema}`;
    ...
    // Contenedor de estadísticas (4 columnas)
    ...
    col1.createDiv({ cls: "stat-column-title", text: "Éxito Mensual" });
    this.renderDonutChart(col1, advancedStats.monthSuccess, habit.colors);

    // Columna 2: Día más productivo
    const col2 = statsContainer.createDiv({ cls: "stat-column" });
    col2.createDiv({ cls: "stat-column-title", text: "Día Más Productivo" });
    ...
    topDayInfo.createDiv({
      cls: "top-day-percent",
      text: `${advancedStats.topDayPercent}% de éxito`,
    });
    ...
    if (advancedStats.trend === "up") trendArrow.textContent = "↑";
    else if (advancedStats.trend === "down") trendArrow.textContent = "↓";
    else trendArrow.textContent = "→";
    trend.createSpan({
      text:
    ...
    for (const badge of advancedStats.badges) {
      ...
      console.log(`📍 Loading badge icon: ${iconImg.src}`);

      // Manejo de error si la imagen no carga
      iconImg.addEventListener("error", () => {
        console.error(`❌ Failed to load icon: ${badge.icon}`);
      ...
      iconImg.addEventListener("load", () => {
        console.log(`✅ Icon loaded successfully: ${badge.icon}`);
      });
  ...
  /**
   * Renderiza un gráfico de dona (donut chart)
   */
  renderDonutChart(container, percentage, colors) {
    ...
    centerText.createDiv({ cls: "donut-label", text: "Éxito" });
  ...
  /**
   * Renderiza las estadísticas del hábito (VERSIÓN CORREGIDA)
   */
  renderStats(card, habit, stats) {
    ...
    // Cálculo seguro del porcentaje (evita división por cero)
    const rawPercentage =
    ...
    // 4. Lógica de color robusta
    // Intentamos usar el 3er color del hábito, o el último disponible, o verde por defecto.
    ...
    totalStat.createSpan({ text: `Total: ${stats.totalDays} días` });

    const bestStreakStat = statsContainer.createDiv({ cls: "stat-item" });
    bestStreakStat.createSpan({
      text: `Mejor racha: ${stats.bestStreak} días`,
    });
  }
  /**
   * Muestra mensaje cuando no hay configuración
   */
